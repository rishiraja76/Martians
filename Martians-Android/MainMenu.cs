...
void OnGUI () 
	...
	// Toggle _play on Inventory button click.
		if (GUI.Button (new Rect (Screen.width / 2 - 27,255,75, 20), "Play")) 
	{
		_play = !_play;
		...
		if (GUI.Button (new Rect (Screen.width / 2 - 27, 285, 75, 20), "Info :"))
	{
			_info = !_info;
			GetComponent<AudioSource>().PlayOneShot(click);
		...
		if (GUI.Button (new Rect (Screen.width / 2 - 27, 315, 75, 20), "Exit"))
		{
			Debug.Break();
			GetComponent<AudioSource>().PlayOneShot(click);
